...
### 2.2 IN_PROGRESS 세션 자동 정리 (서버 idle timeout)

서버는 `IN_PROGRESS` 상태로 일정 시간 이상 진행이 없는 세션을 자동으로 `ABORTED`로 정리합니다.

| 항목 | 값 |
|---|---|
| 현재 idle timeout | **60분** (단축 예정) |
| 트리거 | `IN_PROGRESS` 진입 후 결과 제출/ABORT 없이 일정 시간 경과 |
| 결과 | 세션 자동 `ABORTED` 전이 |

운영 중 PC 강제 종료, 네트워크 끊김 등으로 클라이언트가 `SESSION_ABORT`를 보내지 못해도 일정 시간 후 서버가 자동 정리하므로 다음 방문자가 영구히 막히는 일은 없습니다. 다만 그 사이엔 같은 visitorCode로 새 세션 생성이 거부될 수 있어 개발 중에는 §4.2 Dev reset API로 우회.
...
| `STARTED_BEFORE_CREATED` | `startedAt`이 세션 생성 시각 이전 — **2026-05-13 검증 제거 이후 발생 안 함** | (legacy) |
| `INVALID_STARTED_AT` | `startedAt` 형식/범위 이상 — **2026-05-13 검증 제거 이후 발생 안 함** | (legacy) |
...
> **2026-05-13 업데이트 — 시간 검증 제거됨**
> 백엔드가 `startedAt` 시간 검증 로직(`STARTED_BEFORE_CREATED` / `INVALID_STARTED_AT`)을 **제거**했습니다. 이제 어떤 값을 보내도 NACK이 안 납니다.
> - 안전 마진 코드 (`AddSeconds(-2)`, `AddMilliseconds(-500)` 등) **모두 불필요** — 발견하면 `DateTime.Now` 그대로로 정리하세요.
> - 형식 명세는 그대로 유지 (ISO-8601 LocalDateTime + 밀리초). 미래에 백엔드가 다시 검증을 추가할 가능성에 대비.
...
| 2026-05-13 | **`startedAt` 시간 검증 제거 (백엔드 변경)** — `STARTED_BEFORE_CREATED`/`INVALID_STARTED_AT` NACK 더 이상 발생 안 함. 안전 마진 코드(`AddSeconds(-2)` 등) 모두 불필요. **IN_PROGRESS 세션 idle timeout 60분 정책 명시** (§2.2 신규, 추후 5~10분으로 단축 예정). §3.3 NACK 표, §12.4 ②번 업데이트. 모든 부스 클라이언트에 영향 — 안전 마진 코드 있으면 `DateTime.Now` 그대로로 정리 권장. |
